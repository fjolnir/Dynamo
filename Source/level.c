...
Level_t *level_load(const char *aFilename)
{
	...
	if(characterObjGroup) {
		...
		out->character->sprite->animations[4] = sprite_createAnimation(4);
		out->character->sprite->animations[4].loops = false;
		out->character->sprite->animations[5] = sprite_createAnimation(2);
		out->character->sprite->animations[5].loops = false;
...
static void _level_draw(Renderer_t *aRenderer, void *aOwner, double aTimeSinceLastFrame, double aInterpolation)
{
	...
	center.x = CLAMP(center.x, 0.0f, level->size[0]*level->tileSize.w);
	center.y = CLAMP(center.y, 0.0f, level->size[1]*level->tileSize.h);
