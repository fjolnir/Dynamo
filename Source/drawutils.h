...
/*!
	Frees all resources used by the drawing utilities
*/
...
/*!
	Draws multiple subtextures at different locations using a texture atlas. (In a single draw call, useful for performing multiple simple draws, such as when drawing a tiled level)
	@param aOffsets an array of [x,y] offsets in the texture atlas (Cast to int)
	@param aCenterPoints an array of points to draw the tiles at
*/
extern void draw_textureAtlas(TextureAtlas_t *aAtlas, int aNumberOfTiles, vec2_t *aOffsets, vec2_t *aCenterPoints);

/*!
	Generates the vertices used by the above function (use this if you want to store your vertices in a VBO when handling larger meshes.
*/
...
/*!
	Iterates a game world, drawing every shape in it.
*/
...
/*!
	Iterates a world entity, drawing every shape in it.
*/
